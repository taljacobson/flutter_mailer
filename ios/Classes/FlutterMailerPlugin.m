...
- (void)send:(mail:(NSDictionary *)options
                  callback: (FlutterResult)callback)
{
    if ([MFMailComposeViewController canSendMail])
    {
        ...
        if (options[@"subject"]){
            NSString *subject = [ NSString:options[@"subject"]];
        ...
        if (options[@"body"]){
            NSString *body = [ NSString:options[@"body"]];
            [mail setMessageBody:body isHTML:isHTML];
        }

        if (options[@"recipients"]){
            NSArray *recipients = [ NSArray:options[@"recipients"]];
            [mail setToRecipients:recipients];
        }

        if (options[@"ccRecipients"]){
            NSArray *ccRecipients = [ NSArray:options[@"ccRecipients"]];
            [mail setCcRecipients:ccRecipients];
        }
        
        if (options[@"bccRecipients"]){
            NSArray *bccRecipients = [NSArray:options[@"bccRecipients"]];
            [mail setBccRecipients:bccRecipients];
        }

        if (options[@"attachment"]){
            NSArray *attachments = [ NSArray:options[@"attachments"]];
            for (NSString *attachmentPath in attachments) {
              NSLog(@"Single element: %@", attachmentPath);
              ...
              NSString *attachmentName = [[attachmentPath lastPathComponent] stringByDeletingPathExtension];
...
- (void)mailComposeController:(MFMailComposeViewController *)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError *)error
{
    ...
    //     }
        ...
        NSLogWarn(@"No callback registered for mail: %@", controller.title);
    }
